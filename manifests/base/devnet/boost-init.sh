...
lotus wait-api --timeout ${LOTUS_WAIT_API_TIMEOUT:-60s}
echo Wait for lotus-miner is ready ...
lotus-miner wait-api --timeout ${LOTUS_WAIT_API_TIMEOUT:-60s}
...
if [ ! -f $BOOST_PATH/.register.boost ]; then
	...
	boostd -vv run --nosync=true & BOOST_PID=`echo $!`
	echo Got boost PID = $BOOST_PID
	
	until boostd net id | grep -q "1"; do echo "Waiting for boost..."; sleep 1; done
